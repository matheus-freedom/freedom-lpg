...
export const generateLessonPlan = async (params: {
  ...
  const result = await callGemini("generateContent", {
    ...
    contents: prompt,
    config: { responseMimeType: "application/json", responseSchema: lessonPlanSchema },
  });
...
export const generateExam = async (level: CEFRLevel): Promise<string> => {
  const result = await callGemini("generateContent", {
    ...
    contents: `Create a comprehensive English exam for CEFR level ${level}.
...
export const generatePlaygroundActivity = async (level: CEFRLevel, typeId: string): Promise<any> => {
  ...
  const result = await callGemini("generateContent", {
    ...
    config: { responseMimeType: "application/json", responseSchema: playgroundSchema },
  });
